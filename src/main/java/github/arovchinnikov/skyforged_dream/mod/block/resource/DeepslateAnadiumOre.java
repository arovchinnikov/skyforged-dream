package github.arovchinnikov.skyforged_dream.mod.block.resource;

import github.arovchinnikov.skyforged_dream.mod.block.base.BlockReference;
...
public class DeepslateAnadiumOre extends Block {
    public DeepslateAnadiumOre() {
        super(BlockReference.STONE.getSettings().strength(3.8f, 6));
    }
    @Override
    ...
    private void triggerCloud(World world, BlockPos pos) {
        ...
        if (existClouds.isEmpty() || existClouds.getFirst().isRemoved()) {
            AnadiumPoisonCloud cloud = new AnadiumPoisonCloud(EntityTypes.ANADIUM_POISON_CLOUD.entityType(), world);
