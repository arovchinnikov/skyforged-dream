...
public class PearlShell extends ModBlock implements Waterloggable {
    public static final DirectionProperty FACING = HorizontalFacingBlock.FACING;
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;
    public PearlShell() {
        super(Blocks.LADDER.getSettings().strength(1, 1), ItemRarity.COMMON, true);
    ...
    private ActionResult handUse(World world, BlockPos pos, PlayerEntity player, Random random) {
        ...
        world.playSound(null, pos, SoundEvents.BLOCK_BONE_BLOCK_HIT, SoundCategory.BLOCKS, 1.f, .8f);
        if (0.5 > random.nextDouble()) {
            player.damage(world.getDamageSources().inWall(), random.nextBetween(1, 3));
        }

        return ActionResult.PASS;
    }

    private void successOpen(World world, BlockPos pos, PlayerEntity player) {
        world.breakBlock(pos, false);
        world.playSound(null, pos, SoundEvents.BLOCK_BONE_BLOCK_BREAK, SoundCategory.BLOCKS, 1.f, .8f);

        player.addExperience(2);

        if (0.1 > Random.create().nextDouble()) {
            ItemScatterer.spawn(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(Items.BLACK_PEARL.get()));
        } else {
            ItemScatterer.spawn(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(Items.PEARL.get()));
        }
    }
